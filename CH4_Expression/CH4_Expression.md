...
---
❗️关于优先级注意：
`[]`下标运算符 的 优先级极高(和`()`优先级相同)，比定义指针和解引用的`*`运算符优先级高！所以下面定义的实际含义请特别注意：

```
1： int arr[10];		// arr 是一个 含有10个(int)整数的 数组
2： int *arr[10];		// arr 是一个 含有10个(int*)指针的 数组
3： int (*arr)[10];	// arr 是一个 指向含有10个整数数组的 指针！
```
解释：由于 `[]`运算符比`*`运算符 的优先级要更高，所以第二句定义，首先 arr 被`[]`运算符绑定定义变成了一个**数组**！而第三句中强行使用`()`运算符将`*`运算符优先级提高，所以这里的 arr 就被首先确定为一个**指针**而非数组。
